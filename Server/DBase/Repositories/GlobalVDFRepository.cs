...
{
    public interface IVDFRepository
    {
        Task<List<GViewDefinitionFileBase>> GetVDFList();
        Task<TextObject> GetVDF(int id);
        Task DeleteVDF(int id);
        Task<GViewDefinitionFile> AddVDF(GViewDefinitionFile vdf);
        Task<GViewDefinitionFile> UpdateVDF(GViewDefinitionFile vdf);

    }

    public interface IGlobalVDFRepository : IVDFRepository { }

    public class GlobalVDFRepository : IGlobalVDFRepository
    {
        private readonly GlobalDbContent dbContext;

        public GlobalVDFRepository(GlobalDbContent dbContext)
        {
        ...
        {
            return await dbContext.VdfObjs
